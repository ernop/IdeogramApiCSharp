...
namespace IdeogramAPIClient
{
    public static class TextUtils
    {
        ...
        public static async Task SaveImageAndAnnotateText(byte[] imageBytes, List<Tuple<string, string>> texts, Dictionary<string, string> imageInfo, string outputPath)
        {
            ...
            using (var graphics = Graphics.FromImage(annotatedImage))
            {
                ...
                using (var font = new Font("Arial", 12, FontStyle.Regular))
                {
                    ...
                    {
                        DrawKeyValuePair(graphics, text.Item1, text.Item2, font, leftMargin, keyWidth, valueWidth, ref y);
        ...
        private static int CalculateTextHeight(List<Tuple<string, string>> texts, Dictionary<string, string> imageInfo, int imageWidth)
        {
            int height = 0;
            using (var font = new Font("Arial", 12, FontStyle.Regular))
            ...
            using (var graphics = Graphics.FromImage(bitmap))
            {
                ...
                {
                    SizeF keySize = graphics.MeasureString(textPair.Item1, font, new SizeF(keyWidth, float.MaxValue), stringFormat, out int keyCharsFitted, out int keyLinesFilled);
                    SizeF valueSize = graphics.MeasureString(textPair.Item2, font, new SizeF(valueWidth, float.MaxValue), stringFormat, out int valueCharsFitted, out int valueLinesFilled);
